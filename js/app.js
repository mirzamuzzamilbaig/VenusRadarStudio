...
// ─── Static / GitHub Pages Mode Auto-detection ────────────────────────────────
const isGitHubPages = window.location.hostname.endsWith('github.io') ||
                      window.location.protocol === 'file:' ||
                      window.location.search.includes('static=true');
let useStaticData = isGitHubPages;

const API = {
  basemap: () => useStaticData ? 'data/basemap.png' : '/api/basemap',
  cpr: () => useStaticData ? 'data/cpr_layer.png' : '/api/cpr_layer',
  async fetch(endpoint, staticFallback) {
    if (!useStaticData) {
      try {
    ...
    const res = await fetch(staticFallback || `data/${endpoint}.json`);
    return await res.json();
  }
};

function computeClientCPRStats(lat, lon, radiusDeg = 5.0) {
  let minDist = Infinity;
  let nearest = null;
  if (State.hotspots && State.hotspots.length > 0) {
    for (const f of State.hotspots) {
      const [fLon, fLat] = f.geometry.coordinates;
      const d = Math.hypot(fLat - lat, fLon - lon);
      if (d < minDist) {
        minDist = d;
  ...
  let seed = Math.floor(Math.abs(lat * 137 + lon * 97)) % 9999;
  ...
  let centerCpr;
  if (nearest && minDist < (nearest.radius_deg || 5)) {
    const t = 1.0 - minDist / nearest.radius_deg;
    centerCpr = 0.14 + (nearest.cpr_peak - 0.14) * t;
  } else {
    centerCpr = 0.14 + pseudoRandom() * 0.08;
  }

  const n = 1200;
  const scale = centerCpr / 2.0;
  const samples = [];
  let sum = 0;
  for (let i = 0; i < n; i++) {
    const u1 = Math.max(1e-7, pseudoRandom());
    const u2 = Math.max(1e-7, pseudoRandom());
    let val = -scale * (Math.log(u1) + Math.log(u2));
    val = Math.max(0.01, Math.min(3.5, val));
    samples.push(val);
    sum += val;
  }
  ...
  const binWidth = 2.5 / numBins;
  for (let b = 0; b <= numBins; b++) binEdges.push(parseFloat((b * binWidth).toFixed(3)));
  ...
  let terrainClass = 'Plains';
  if (mean < 0.20) terrainClass = 'Plains';
  else if (mean < 0.45) terrainClass = 'Lava Fields';
  else if (mean < 0.75) terrainClass = 'Rough Volcanic';
  else if (mean < 1.10) terrainClass = 'Active Flow Zone';
  else terrainClass = 'Retroreflector (Highland)';

  let interp = '';
  if (mean < 0.20) interp = 'Smooth volcanic plains — basaltic lowlands, minimal surface roughness at 12.6 cm wavelength';
  else if (mean < 0.40) interp = 'Moderately rough terrain — pahoehoe-type lava flows or fractured plains';
  else if (mean < 0.70) interp = 'Rough volcanic surface — aa-type lava flows or deformed tesserae terrain';
  else if (mean < 1.10) interp = 'Highly rough — probable recent/active volcanic flow field (similar to Maat Mons)';
  else interp = 'Extreme retroreflection — mountain/highland terrain with specular coherent backscatter (Maxwell-type)';

  return {
    lat: Number(lat.toFixed(4)),
    lon: Number(lon.toFixed(4)),
    radius_deg: radiusDeg,
    mean_cpr: Number(mean.toFixed(4)),
    median_cpr: Number(median.toFixed(4)),
    max_cpr: Number(max.toFixed(4)),
    std_cpr: Number(std.toFixed(4)),
    p95_cpr: Number(p95.toFixed(4)),
    histogram: { counts, bin_edges: binEdges },
    interpretation: interp,
    terrain_class: terrainClass,
    nearest_feature: nearest ? nearest.name : 'Open Plains',
    distance_to_feature_deg: Number(minDist.toFixed(2))
  };
}
...
const VenusMap = (() => {
  ...
  function init() {
    ...
    // ── CPR overlay (RGBA) ─────────────────────────────────────────────────
    State.layers.cpr = L.imageOverlay(API.cpr(), baseBounds, {
...
const CPRChart = (() => {
  ...
  async function fetchAndUpdate(lat, lon) {
    ...
    const data = computeClientCPRStats(lat, lon);
    update(data);
  }
...
async function init() {
  ...
  Loader.setProgress(20, 'Connecting to radar data engine…');
  try {
    const probe = await fetch('/api/status', { method: 'HEAD' });
    if (!probe.ok) useStaticData = true;
  } catch (e) {
    useStaticData = true;
  }
  await sleep(100);
  ...
  try {
    ...
    const [hotspotsGJ, changesGJ, papers, terrain] = await Promise.all([
      API.fetch('hotspots', 'data/hotspots.json'),
      API.fetch('changes', 'data/changes.json'),
      API.fetch('literature', 'data/literature.json'),
      API.fetch('terrain_data', 'data/terrain_data.json')
    ]);
